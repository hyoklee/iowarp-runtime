...
struct DomainMapEntry {
  ...
  SubDomainId &Get(ContainerId hash) {
    if (size_ > 0) {
      ...
      HELOG(kError, "DomainMapEntry is empty");
      throw std::runtime_error("DomainMapEntry is empty");
    }
  }
};
 ...
 public:
  ...
  SubDomainId GetSubDomainId(const PoolId &scope,
    ...
    if (dom_query.flags_.Any(DomainQuery::kId)) {
      ...
      auto it = domain_map_.find(major_id);
      if (it == domain_map_.end()) {
        HELOG(kError, "Domain {} not found", major_id);
        throw std::runtime_error("Domain not found");
      }
      DomainMapEntry &major_entry = it->second;
  ...
  std::vector<ResolvedDomainQuery> ResolveDomainQuery(
    ...
    if (dom_query.flags_.All(DomainQuery::kLocal | DomainQuery::kId)) {
      ...
      HELOG(kError, "(node {}) Unknown domain query pool={} type: {}", node_id_,
            scope, dom_query);
    }
    return res;
  }
